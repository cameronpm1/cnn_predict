...
class VisDynamicsModel(pl.LightningModule):
    ...
    def __init__(self,
                 ...
                 dataset: str='cube_sim',
                 lr_schedule: list=[20, 50, 100],
                 input_type: str='step') -> None:
        super().__init__()
    ...
    def __build_model(self):
        ...
        if self.hparams.model_name == 'refine-64' and 'cube_2d' in self.hparams.dataset:
            self.model = RefineCube2DModel(in_channels=64)
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit':
            ...
            if 'refine' in self.hparams.model_name:
                ...
                if self.hparams.input_type == 'step':
                    self.train_dataset = DynamicImageDatasetStep(**kwargs)
                    kwargs['flag'] = 'val'
                    self.val_dataset = DynamicImageDatasetStep(**kwargs)
                elif self.hparams.input_type == 'constant':
                    self.train_dataset = DynamicImageDatasetConstant(**kwargs)
                    kwargs['flag'] = 'val'
                    self.val_dataset = DynamicImageDatasetConstant(**kwargs) 
                else:
        ...
        if stage == 'test':
            ...
            if self.hparams.input_type == 'step':
                self.test_dataset = DynamicImageDatasetStep(**kwargs)
            elif self.hparams.input_type == 'constant':
                self.test_dataset = DynamicImageDatasetConstant(**kwargs)
            else:
                    print('Incorrect data type input')
                    exit()
