...
class VisLatentDynamicsModel(pl.LightningModule):
    ...
    def setup(self, stage=None):
        if stage == 'fit':
            self.train_dataset = DynamicImageDatasetStep(data_filepath=self.hparams.data_filepath,
                                                    flag='train',
                                                    seed=self.hparams.seed,
                                                    object_name=self.hparams.dataset)
            self.val_dataset = DynamicImageDatasetStep(data_filepath=self.hparams.data_filepath,
                                                    flag='val',
                                                    seed=self.hparams.seed,
                                                    object_name=self.hparams.dataset)

        if stage == 'test':
            self.test_dataset = LatentDynamicImageDatasetStep(data_filepath=self.hparams.data_filepath,
                                                                 flag='test',
                                                                 seed=self.hparams.seed,
                                                                 object_name=self.hparams.dataset)
