...
class DynamicImageDatasetStep(Dataset):
    def __init__(self, data_filepath, flag, seed, object_name="cube_2d"):
        self.seed = seed
        self.flag = flag
    ...
    def get_all_filelist(self):
        ...
        for vid_idx in vid_list:
            ...
            for p_frame in range(num_frames - 4):
                par_list = []
                for p in range(4):
...
class DynamicImageDatasetConstant(DynamicImageDatasetStep):
    def __init__(self, data_filepath, flag, seed, object_name="cube_2d"):
        super().__init__(data_filepath, flag, seed, object_name="cube_2d")

    #0 -> 0
    def __getitem__(self, idx):
        par_list = self.all_filelist[idx]
        data = []
        data.append(self.get_data(par_list[0])) # 0
        target = []
        target.append(self.get_data(par_list[0])) # 0
        filepath = '_'.join(par_list[0].split('/')[-2:])
        return data, target, filepath
    ...
    def get_all_filelist(self):
        ...
        # go through all the selected videos and get the single frame: input(t)
        for vid_idx in vid_list:
            seq_filepath = os.path.join(obj_filepath, str(vid_idx))
            num_frames = len(os.listdir(seq_filepath))
            suf = os.listdir(seq_filepath)[0].split('.')[-1]
            for p_frame in range(num_frames):
                par_list = []
                par_list.append(os.path.join(seq_filepath, str(p_frame) + '.' + suf))
                filelist.append(par_list)
        return filelist



class LatentDynamicImageDatasetStep(Dataset):
    def __init__(self, data_filepath, flag, seed, object_name="cube_2d"):
        self.seed = seed
        self.flag = flag
        self.object_name = object_name
